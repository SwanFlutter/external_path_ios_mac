...
class ExternalPathIosMac {
  ...
  Future<String?> getDirectoryPath({required DirectoryType directory}) {
    return ExternalPathIosMacPlatform.instance.getDirectoryPath(
  ...
  Future<String?> getDirectoryPathMacOs({required MacDirectoryType directory}) {
    return ExternalPathIosMacPlatform.instance.getDirectoryPathMacOs(
      directory: directory.value,
    );
  }
  ...
  /// Retrieves the root installation path where applications are installed.
  ///
  /// This method calls the `getRootInstallationPath` method of the current platform-specific
  /// implementation of [ExternalPathIosMacPlatform]. On iOS, this typically returns the
  /// application's bundle path or sandbox root. On macOS, this returns the application's
  /// installation directory.
  ///
  /// Returns:
  ///   A [Future] that completes with the root installation path as a [String], or `null`
  ///   if the path is not available.
  ///
  /// Example:
  /// ```dart
  ...
  ///   try {
  ///     String? path = await ExternalPathIosMac().getRootInstallationPath();
  ...
  Future<String?> getRootInstallationPath() {
    return ExternalPathIosMacPlatform.instance.getRootInstallationPath();
  }
}
